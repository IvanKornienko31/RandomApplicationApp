...
Далее будут приведены добавленные фрагменты в файлы `libs.versions.toml` и `build.gradle.kts` для
модуля `:composeApp`
...
С этим экраном возникла проблема: из-за viewModel блок из поля ввода перерисовывается больше раз,
чем до его внедрения. Решений проблемы не нашел.

Данные картинки отражают состояние, при котором в поле логина введено "data@data.com", а в поле
пароля - "1234567890"
...
2. **Реальные устройства**:
    - Xiaomi Redmi 12 (API 33, Android 13 _with MIUI 14_). Идентификатор устройства: Xiaomi
      23053RN02A
